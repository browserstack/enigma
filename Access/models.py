...
class User(models.Model):
    ...
    bot_type = models.CharField(max_length=100, null=False, blank=False, 
                            choices=BOT_TYPES, default='None')
    ...
    ssh_public_key = models.ForeignKey('SshPublicKey', related_name='user', blank=True, null=True, on_delete=models.PROTECT)

    avatar = models.TextField(null=True,blank=True)

    state = models.CharField(max_length=255, null=False, blank=False, choices=USER_STATUS_CHOICES, default=1)
    role = models.ManyToManyField(Role, blank=True)

    offbaord_date = models.DateTimeField(null=True, blank=True)
    revoker = models.ForeignKey('User', null=True, blank=True,related_name="user_revoker", on_delete=models.PROTECT)

    @property
    def permissions(self):
        user_roles = self.role.all()
        user_permissions = [permission for role in user_roles for permission in role.permission.all()]
...
class MembershipV2(models.Model):
    ...
    """
    membership_id = models.CharField(max_length=255, null=False, blank=False, unique=True)

    user = models.ForeignKey('User',null=False, blank=False, related_name="membership_user", on_delete=models.PROTECT)
    group = models.ForeignKey('GroupV2',null=False, blank=False, related_name="membership_group", on_delete=models.PROTECT)
    is_owner = models.BooleanField(null=False, blank=False, default = False)

    requested_by = models.ForeignKey(User,null=False,blank=False, related_name="membership_requester", on_delete=models.PROTECT)
    ...
    status = models.CharField(max_length=255, null=False, blank=False, choices=STATUS,default='Pending')

    reason = models.TextField(null=True,blank=True)

    approver = models.ForeignKey('User',null=True, blank=True, related_name="membership_approver", on_delete=models.PROTECT)
...
class GroupV2(models.Model):
    ...
    requester = models.ForeignKey('User', null=True, blank=True, related_name="group_requester", on_delete=models.PROTECT)
    ...
    approver = models.ForeignKey('User', null=True, blank=True, related_name="group_approver", on_delete=models.PROTECT)
    decline_reason = models.TextField(null=True, blank=True)
...
class gitAcces(models.Model):
    ...
    requestId=models.CharField(max_length=255, null=False, blank=False,primary_key=True)
...
class UserAccessMapping(models.Model):
    ...
    access = models.ForeignKey('AccessV2', null=False, blank=False, on_delete=models.PROTECT)
    ...
    access_type = models.CharField(max_length=255, null=False, blank=False, 
                                choices=TYPE_CHOICES, default='Individual')
    revoker = models.ForeignKey('User', null=True, blank=True,related_name="user_access_revoker", on_delete=models.PROTECT)

    def __str__(self):
        return self.request_id

    # Wrote the override version of save method in order to update the "approved_on" field whenever the request is marked "Approved"
...
class GroupAccessMapping(models.Model):
    ...
    group = models.ForeignKey('GroupV2', null=False, blank=False, on_delete=models.PROTECT)

    requested_by = models.ForeignKey('User', null=True, blank=False, related_name="g_requester", on_delete=models.PROTECT)

    request_reason = models.TextField(null=False, blank=False)

    approver_1 = models.ForeignKey('User', null=True, blank=True,related_name="g_approver_1", on_delete=models.PROTECT)
    approver_2 = models.ForeignKey('User', null=True, blank=True, related_name="g_approver_2", on_delete=models.PROTECT)
    ...
    revoker = models.ForeignKey('User', null=True, blank=True,related_name="group_access_revoker", on_delete=models.PROTECT)
    ...
    def __str__(self):
        try:
            if self.access_tag == "aws":
                ...
                return "{}: Team- {} | Access: {} | Level: {} | Service: {} | Resource: {}".format(self.access_tag, label['team'], label['awsAccessType'], label['levelAccessType'], label['awsService'], label['awsResource'])
