...
def createGroup(request):
    try:
        ...
        if (
            "requiresAccessApprove" not in data
        ...
        body = helpers.generateStringFromTemplate(
            filename="email.html",
            emailBody=generateNewGroupCreationEmailBody(
                request,
                group_id,
                newGroupName,
                initialMembers,
...
def updateOwner(request, group, context):
    ...
    # we will only get data["owners"] as owners who are checked in UI(exluding disabled
...
def add_user_to_group(request):
    try:
        ...
        context["status"] = {
            ...
            "msg": (
                "Once Approved the newly added members will be granted the same"
