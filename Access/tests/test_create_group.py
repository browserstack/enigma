...
from Access.group_helper import (
    create_group,
...
def group_already_exists(mocker):
    mocker.patch(
        "Access.models.GroupV2.group_exists", return_value=[mocker.MagicMock()]
    )
...
def send_notification(mocker):
    mocker.patch(
        "Access.notifications.send_new_group_create_notification", return_value=""
    )
