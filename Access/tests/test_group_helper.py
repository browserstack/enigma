...
@pytest.mark.parametrize(
    (
        "testname,postData,expectedContext, membershipCreationCallCount,"
        " needsAccessApprove"
    ),
    [
        (
            ...
            "{'error': {'error_msg': 'Invalid Group Name', 'msg': \"A group with"
            " name ['"
            + testGroupName
        ...
        (
            ...
            "newGroupName=['"
            + testGroupName
            + "']&requiresAccessApprove=false&newGroupReason=['test reason']",
            "{'status': {'title': 'New Group Request submitted', 'msg': \"A request for"
            " New Group with name ['"
            + testGroupName
            + "'] has been submitted for approval. You will be notified for any changes"
            ' in request status."}}',
            1,
            False,
        ),
        (
            ...
            "newGroupName=['"
            + testGroupName
            + "']&requiresAccessApprove=false&newGroupReason=['test"
            " reason']&selectedUserList=user1@user1.com",
            "{'status': {'title': 'New Group Request submitted', 'msg': \"A request for"
            " New Group with name ['"
            + testGroupName
            + "'] has been submitted for approval. You will be notified for any changes"
        ...
        (
            ...
            "newGroupName=['"
            + testGroupName
            + "']&requiresAccessApprove=true&newGroupReason=['test"
            " reason']&selectedUserList=user1@user1.com",
            "{'status': {'title': 'New Group Request submitted', 'msg': \"A request for"
            " New Group with name ['"
            + testGroupName
...
def test_createGroup(
    ...
    if testname == GroupAlreadyExists:
        ...
        mocker.patch(
            "Access.models.GroupV2.objects.filter", return_value=statusFilterPatch
        )

    elif testname == GroupDoesNotExists_DoesNotNeedsApproval_1:

        def mockGenerateNewGroupCreationEmailBody(
            request,
            group_id,
            newGroupName,
            initialMembers,
        ...
        group_helper.generateNewGroupCreationEmailBody = (
            mockGenerateNewGroupCreationEmailBody
        )
    ...
    if membershipCreationCallCount > 0:
        assert (
            models.MembershipV2.objects.create.call_count == membershipCreationCallCount
        )
        models.GroupV2.objects.create.assert_called_with(
            name=ANY,
            group_id=ANY,
            requester=ANY,
            description=ANY,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (
                "{'error': 'Error Occured while Approving group creation. Please"
                " contact admin - sendEmailError'}"
            ),
            "grp1",
            False,
            True,
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "TestGroupName1",
            (
                "{'status': {'title': 'Invalid Group', 'msg': 'There is no group named"
                " TestGroupName1. Please contact admin for any queries.'}}"
            ),
        ),
        (
            ...
            "TestGroupName1",
            (
                "{'error': {'error_msg': 'Internal Error', 'msg': \"Error Occured while"
                " loading the page. Please contact admin, Permission denied, you're not"
                ' owner of this group"}}'
            ),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            (
                "{'error': {'error_msg': 'Internal Error', 'msg': 'Error Occured while"
                " loading the page. Please contact admin, Permission denied, requester"
                " is non owner'}}"
            ),
        ),
        (
            ...
            (
                "{'error': {'error_msg': 'Duplicate Request', 'msg': 'User"
                " member2@member2.com is already added to group/or pending approval for"
                " group addition'}}"
            ),
        ),
        (
            ...
            (
                "{'status': {'title': 'Request Submitted', 'msg': 'Once Approved the"
                " newly added members will be granted the same permissions as the"
                " group'}}"
            ),
        ),
        (
            ...
            "groupName=TestGroupName&selectedUserList=member2@member2.com&memberReason=somereason",
            (
                "is now being processed', 'desc': 'A email will be sent after the"
                " requested access are granted'}}"
            ),
        ),
    ],
)
