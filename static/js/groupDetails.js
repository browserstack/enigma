...
function remove_user_from_user_confirmed(btnElem) {
  ...
  const membership_id = $(btnElem).attr("id");
  ...
  $.ajax({
    ...
    success: function(data) {
      document.getElementById(membership_id+"-remove-user-button").innerHTML = "";
      document.getElementById(membership_id+"-user-state").innerHTML = "revoked";

      checkbox_elem = document.getElementById(membership_id+"-user-checkbox")
...
$(document).on('click', '.group-revoke-button', function(){
    ...
    $.ajax({url: urlBuilder,
        ...
        success: function(result){
          revoke_button_elem = $(document.getElementById("inactive-"+id+"-revoke-button")).children()[0];
          $(revoke_button_elem).hide();
          status_elem = document.getElementById("access-status-" + id);
