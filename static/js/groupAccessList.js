const focusActiveTab = (elem) => {
  ...
  console.log(inputElem.attr('value'))
  const active = document.getElementsByClassName("active");
  if (active != inputElem) {
    $(active).removeClass("active text-blue-600 border-blue-600 dark:text-blue-500 dark:border-blue-500")
      .addClass("border-transparent hover:text-gray-600 hover:border-gray-400 dark:hover:text-gray-400");
    $(`#${inputElem.attr('show')}`).removeClass('hidden');
    $(`#${inputElem.attr('hide')}`).addClass('hidden');
    $(inputElem).removeClass("border-black hover:text-gray-600 hover:border-gray-400 dark:hover:text-gray-400")
